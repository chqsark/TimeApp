...
window.taskTimer.ui = (function ($, model, utils) {
    ...
    var buildDurationDiffHtml = function (taskDurationDs) {
        if (taskDurationDs < 0) return -1;
        else return taskDurationDs;
    ...
    var createTaskListItem = function (task) {
        ...
        durationElement.attr('class', 'daysCountdown'); //ui-li-count duration');
        durationElement.attr('style', 'font-size:21px;');
        //durationElement.html(buildDurationHtml(task.getTotalDurationMs()));
        durationElement.html(buildDurationDiffHtml(task.getTotalDurationDs()));
    ...
    var addNewTask = function (name, date) {
        uiDurationUpdateEnabled = false;
        
        var task = model.addTask(name, date);
        model.saveToLocalStorage();
    ...
    var refreshActiveTaskDurationDisplayFromModel = function () {
        ...
        if ($('#tasksPage').is(':visible')) {
            if (activeItemCache.task != null && activeItemCache.durationElement != null) {
                ...
                */
                var taskDurationDs = activeItemCache.task.getTotalDurationDs();
                activeItemCache.durationElement.html(buildDurationDiffHtml(taskDurationDs));
    ...
    var addTaskPageInit = function () {
        ...
        var bindButtonEvents = function () {
            ...
            $('#addTaskSaveButton').on('click', function () {
                addNewTask($('#addTaskName').val(), $('#addTaskTime').val());
            });
