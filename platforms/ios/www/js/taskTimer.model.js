...
window.taskTimer.model = (function () {
    ...
    var newTask = function (id, name, date, activePeriods) {
        if (name.trim() == '') {
            name = 'no name';
        }
    ...
    var loadFromLocalStorage = function () {
        if (localStorage.tasks) {
            ...
            persistedModel.tasks.forEach(function(persistedTask) {
                ...
                var task = newTask(persistedTask.id, persistedTask.name, persistedTask.date, activePeriods);
    ...
    var addTask = function (plan, date) {
        var id = newGuid();
        var activePeriods = [];
      //  activePeriods.push(date);
        var task = newTask(id, plan, date, activePeriods);
        model.tasks[task.getId()] = task;
